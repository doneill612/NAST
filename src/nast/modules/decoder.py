...
class Decoder(nn.Module):
    def __init__(self, config: NastTransformerConfig):
        ...
        self.blocks = nn.ModuleList(
            [DecoderBlock(config) for _ in range(config.encoder_blocks)]
        )
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        attention_mask: Optional[FloatTensor] = None,
        return_attention: bool = False,
    ) -> Union[FloatTensor, Tuple[FloatTensor, FloatTensor, FloatTensor]]:
        # create positional encoded to be used for temporal attention mechanism
        positions = torch.arange(0, self.config.prediction_length, dtype=torch.long).to(
        ...
        decout = torch.matmul(attn_st, decout.transpose(1, 2).contiguous())
